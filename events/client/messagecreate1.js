...
module.exports = {
  ...
  async execute(message) {
    ...
    const pickingSession = require('../voice/voiceStateUpdate').pickingSession;
    if (!pickingSession) {
      return message.reply('No active picking session!');
    }

    const target = message.mentions.members.first();
    if (!target) return message.reply('Please mention a player! Example: `!pick @User`');
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: 'Available', value: pickingSession.available.length ? pickingSession.available.map(p => p.toString()).join('\n') : 'None', inline: false },
    ...
    if (pickingSession.picksLeft === 0) {
      ...
      setTimeout(() => {
        require('../voice/voiceStateUpdate').pickingSession = null;
      }, 10000);
    }
  }
};
